...
<?php
                    ...
                    $update="UPDATE productionvalues SET ProductionName='$ProductName', quantity='$quantity', UnitPrice='$UnitPrice', textbox5='$TotalCost' WHERE value_ID='$id'";
...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <script src="assets/js/main.js"></script>
    ...
    include "../connect.php";
                $id=$_GET['id'];
                $sqlp="select * from productionvalues WHERE value_ID=$id";
                $results=mysqli_query($conn,$sqlp);
                $num=mysqli_num_rows($results);
                if($num>0)
                {
                    while($row=mysqli_fetch_array($results)){
                        $ProductName=$row['ProductionName'];
                        $quantity=$row['quantity'];
                        $UnitPrice=$row['UnitPrice'];
                        $TotalCost=$row['textbox5'];
                    }
                }
                ?>
 ...
 document.forms["myform"]["TotalCost"].value =(text1*text2);
 }
 
 </script>
  <main id="main" class="main">
                         <span>Edit Added Values</span>
...
<div class="row" style="padding-bottom: 10px;">
    ...
    <div class="col-lg-4 col-sm-12">
        ...
        <div>
        <input type="text" min=1 name="ProductName" value="<?php  echo $ProductName; ?>" class="form-control">
        </div>
    </div>
    <div class="col-lg-4 col-sm-12">
        <label>Quantity:</label>
        <div>
        <input type="number" min=1 name="quantity" class="form-control" value="<?php  echo $quantity; ?>" onkeyup="calculate2(this.value)">
        </div>
    </div>
    <div class="col-lg-4 col-sm-12">
        <label>Unit Price:</label>
        <div>
        <input type="number" step="0.01" min=1 name="UnitPrice" class="form-control" value="<?php  echo $UnitPrice; ?>" onkeyup="calculate2(this.value)">
        </div>
    </div>
        <div class="col-lg-4 col-sm-12">
        <label>Total Cost:</label>
        <div>
        <input type="number" step="0.01" min=1 name="TotalCost" class="form-control" value="<?php  echo $TotalCost; ?>" >
        </div>
    </div>
</div>
                     ...
                     <div class="row">
               <input type="submit" class="form-control" name="update" id="save" value="Save Changes">
      </div>
      </form>
          </main>
    </body>
