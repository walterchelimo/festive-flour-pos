...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <main id="main" class="main">
                  <div>
                      <div class="row">
                          <div class="col">
                      <span>Production Values</span>
                              </div>
                             <div class="col" >
                                 
<?php
if(strpos($permission,'apv')!== false){
?>                            
  <button type="button" style="float:right; margin-right:20px; margin-bottom:10px;" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal"><i class="bi bi-plus-circle"></i>Add-Values</button>
<?php
}
?>
  <!-- Modal -->
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content p-3">
        ...
        <h2 class="modal-title">Add Production Values</h2>
    ...
    include "../connect.php";
                ...
                $name=$_POST['ProductionName'];
                $quantity=$_POST['qty'];
                $UnitCost=$_POST['UnitPrice'];
                $cost=$_POST['textbox5'];
                $sql="insert into productionvalues (ProductionName,quantity,textbox5,UnitPrice) VALUES(' $name','$quantity','$cost','$UnitCost')";
  ...
  <div class="row">
    <div class="col">
        <label>Name:</label>
      <input type="text"  class="form-control" id="name"  name="ProductionName" required placeholder="Name">
    </div>
    <div class="col">    
        <label>Quantity:</label>
      <input type="number" class="form-control" name="qty" min=1 value="N/A" required placeholder="Quantity">
    </div>
          <div class="col">    
        <label>Unit Price:</label>
      <input type="number" step="any" class="form-control" name="UnitPrice" min=1 required placeholder="Unit Price" onkeyup="calculate(this.value)">
    </div>
             <div class="col">    
        <label>Total Cost:</label>
      <input type="number" step="any" class="form-control" name="textbox5" min=1 required placeholder="Total Cost" readonly>
    </div>
        </div>
 ...
 Show  <select name="entries" id="entries">
  ...
  <input class="form-control" id="myInput" type="text" placeholder="Search...">
      ...
      <div class="row">
          <?php 
    include "../connect.php";
    $sql="SELECT * FROM productionvalues ORDER BY value_ID DESC ";
    ...
    if($num_rows21>0){ 
     ...
     while($row=mysqli_fetch_array($results21)){
      echo "<tbody id='myTable'>";
         ...
         echo "<td>".$row['textbox5']."</td>" ;

        echo "<td>";
         if(strpos($permission,'dpv') !== false){
          ?>
          
          <a class='m-2 text-light' href='deleteaddvalues.php?id=<?php echo $row['value_ID'] ?>'><span class='bi bi-trash p-2 bg-danger' style='font-size:20px; border-radius:4px;' >Delete</span></a>
         
          <?php
         }if(strpos($permission,'Epv') !== false){
          ?>
          
          <a class='m-2' href='editaddvalues.php?id=<?php echo $row['value_ID'] ?>'><span class='bi bi-pencil-square bg-primary p-2'  style='font-size:20px; border-radius:4px;'>Edit</span> </a>
         
          <?php
         }if(strpos($permission,'vpv') !== false){
          ?>
          
          <a class='m-2' href='viewaddvalues.php?id=<?php echo $row['value_ID'] ?>'><span class='bi bi-eye bg-info p-2 text-light'  style='font-size:20px; border-radius:4px;'>view </span></a></td>
         </td>
          <?php
         }
