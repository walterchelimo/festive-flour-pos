...
<?php
    ...
    $sql="SELECT * FROM sellingproduction ORDER BY sp_ID DESC LIMIT 10 ";
?>
...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <main id="main" class="main">
     ...
     <span>Recipe </span> Selling Production
  ...
  <div class="card-body">
        <div class="row">
            <div class="col">
<span>Selling price Production</span>
                </div>
                <?php
                if(strpos($permission,'aprr')!== false){
                ?>
            <div class="col">
            <a href="produce.php"><button type="button" class="btn btn-info btn-lg" style="float:right;"><i class="bi bi-plus-circle"></i>Add</button></a>
            </div>
            <?php
            }
            ?>
 ...
 Show  <select name="entries" id="entries">
  ...
  <input class="form-control" id="myInput" type="text" placeholder="Search...">
      ...
      <div class="row">
          <?php 
    include "../connect.php";
    $sql="SELECT * FROM sellingproduction GROUP BY sp_ID ORDER BY sp_ID DESC";
    ...
    if($num_rows21>0){ 
           ...
           echo "<th> Option Values</th>";
           echo "<th>Monthly Expenses</th>";
           echo "<th>Suggested SP</th>";
           ?>
           <th>
             Final SP 
             <?php
              if(strpos($permission,'Eprr')!== false){
              ?>
             <button type="button"  class="btn btn-info" id="btn1" data-toggle="modal" data-target="#myModal" style="width:40px;"><i class="bi bi-plus-circle"></i></button>
              <?php
              }
              ?>
            </th>
    ...
    <div class="modal-content p-3">
      ...
      <div class="modal-body">
          ...
          if(isset($_POST['submit'])){
            ...
            $sqln="SELECT sp from sellingproduction WHERE ProductName='$ProductName'";
            $query=mysqli_query($conn,$sqln);
            ...
            if($numa>0){
              ...
              $insert="UPDATE sellingproduction SET sp='$sp'";
              $query1=mysqli_query($conn,$insert);
              if($query1){
                echo("<script LANGUAGE='JavaScript'>
                window.alert('selling price set');
...
<div class="row mb-3">
  <div class="col">
    ...
    <input type="number" step="any" class="form-control" id="sp" name="sp">
  </div>
      </div>
  ...
  <button type="submit" name="submit"  class="btn btn-primary">Submit</button>
        <button type="button" class="btn btn-default" data-dismiss="modal" style="background-color:gray;color:white;">Close</button>
      </div>
     ...
     while($row=mysqli_fetch_array($results21)){
         ...
         echo "<td>".$row['TotalAddedValues']."</td>" ;
               echo "<td>".$row['TotalMonthlyExpenses']."</td>" ;
               echo "<td>"?><?php $tota = $row['TotalLabourCost']+$row['TotalAddedValues']+$row['BalePrice']-$row['TotalRecycledCost'];
    $count++;
          echo $tota;?> <?php echo "</td>" ;
          ?>
          <td>
            <?php
            if($row['sp']==0){
              $tota = $row['TotalLabourCost']+$row['TotalAddedValues']+$row['BalePrice']-$row['TotalRecycledCost'];
             $count++;
             echo $tota;
            }elseif(($row['sp']>0)){
              echo $row['sp'];
          ...
          <?php
                   if(strpos($permission,'dprr') !== false){
                    ?>
                    <a class='m-2 text-light' href='deleteselling.php?id=<?php echo $row['sp_ID'] ?>'><span class='bi bi-trash p-2 bg-danger' style='font-size:20px; border-radius:4px;' >Delete</span></a>
                    <?php
                   }if(strpos($permission,'Eprr') !== false){
                    ?>
                  <a class='m-2' href='editrecipe.php?id=<?php echo $row['sp_ID'] ?>'><span class='bi bi-pencil-square bg-primary p-2'  style='font-size:20px; border-radius:4px;'>Edit</span> </a>
                    
                    <?php
                   }if(strpos($permission,'vprr') !== false){
                    ?>
                    <a class='m-2' href='viewsellingprice.php?id=<?php echo $row['sp_ID'] ?>'><span class='bi bi-eye bg-info p-2 text-light'  style='font-size:20px; border-radius:4px;'>view </span></a>
                   <?php
                   }
