...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <?php
    ...
    if (isset($_POST['submit'])) {
      ...
      $sql = "UPDATE roles SET RoleName='$roles', permission='$chk' WHERE ID='$ID'";
      $res = mysqli_query($conn, $sql);
      if ($res) {
        echo "<div class='alert alert-success' style='font-size:30px'>Updated Successfully</div>";
      }else{
        echo "<div class='alert alert-danger' style='font-size:30px'>Something went wrong</div>";
      }
    }
    ?>
    <span>Add Role</span>
    <?php
    $id=$_GET['id'];
    $sql11="SELECT * FROM roles WHERE ID='$id'";
    $que=mysqli_query($conn,$sql11);
    $num=mysqli_num_rows($que);
    if($num>0){
        while($arow=mysqli_fetch_array($que)){
        $rolen=$arow['RoleName'];
        }
    }
        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-lg-4 col-sm-12">
              ...
              <input type="text" class="form-control" id="RoleName" value="<?php echo $rolen ?>" name="RoleName" required>
            </div>
