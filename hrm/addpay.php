...
include "../connect.php";
$id=$_GET['id'];
$db="SELECT * FROM casuals Where Casualid='$id'";
...
if($numrow){
    while($row=mysqli_fetch_array($query)){
        ...
        $CasualName=$row['FirstName']." ".$row['LastName'];
        $TotalPaid=$row['TotalPaid'];
        $TotalDue=$row['TotalDue'];
    }

                $sql="select SUM(WagePerPackager) from attendance WHERE AttendanceName LIKE '%$CasualName%'";
                $results=mysqli_query($conn,$sql);
                $num=mysqli_num_rows($results);
                if($num){
                    while($row=mysqli_fetch_array($results)){
                        $TotalAmount=$row['SUM(WagePerPackager)'];
                    }
                }
                if(isset($_POST['addpayment'])){
                    $TotalWage=$_POST['TotalAmount'];
                    $TotalDue=$_POST['Balance'];
                    $Paid=$_POST['Paid'];
                    $Paying=$_POST['Paying'];
                    
                $sql2="UPDATE casuals SET TotalWage='$TotalWage', TotalDue='$TotalDue', TotalPaid='$Paid' WHERE Casualid='$id'";
                $res=mysqli_query($conn,$sql2);
                if($res){
                    $sql3="INSERT INTO payroll (Paying,Balance,Casualid) VALUES('$Paying','$TotalDue','$id')";
                    $result3=mysqli_query($conn,$sql3);
                     echo "<script>window.alert('Payroll Added successfully');
    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="row">
                ...
                <input type="number" step="any" class="form-control" name="TotalAmount" value="<?php echo round($TotalAmount,2)  ?>" readonly onkeyup="calculate(this.value)">
                </div>
                <div class="col-lg-4 col-sm-12">
                <label>Total Due(Ksh)<caps>*</caps></label>
                <input type="number" step="any" class="form-control" name="TotalDue" value="<?php echo $TotalDue?>" readonly onkeyup="calculate(this.value)">
                </div>
                 <div class="col-lg-4 col-sm-12">
                <label>Paid(Ksh)<caps>*</caps></label>
                <input type="number" step="any" class="form-control" name="Paid" value="<?php echo $TotalPaid?>" readonly onkeyup="calculate(this.value)">
                </div>
                <div class="col-lg-4 col-sm-12">
                <label>Paying Amount(Ksh)<caps>*</caps></label>
                <input type="number" step="any" class="form-control" name="Paying" onkeyup="calculate(this.value)">
                </div>
                <div class="col-lg-4 col-sm-12">
                <label>Balance Due:</label>
                <input type="number" step="any" class="form-control" readonly name="Balance" onkeyup="calculate(this.value)">
                    </div>
            </div>
                <div>
                    <input type="submit" name="addpayment" class="btn bg-primary" value="Add Payments" style="float:right;">
                </div>
            </form>
        </div>
    </div>
...
function calculate() {  
if(isNaN(document.forms["myform"]["TotalAmount"].value) || document.forms["myform"]["TotalAmount"].value=="") {   
 var text1 = 0;   
 } else {   
 var text1 = parseInt(document.forms["myform"]["TotalAmount"].value);   
 }
 if(isNaN(document.forms["myform"]["TotalDue"].value) || document.forms["myform"]["TotalDue"].value=="") {   
 var text2 = 0;   
 } else{   
 var text2 = parseInt(document.forms["myform"]["TotalDue"].value);   
 }
 if(isNaN(document.forms["myform"]["Paid"].value) || document.forms["myform"]["Paid"].value=="") {   
 var text3 = 0;   
 } else{   
 var text3 = parseInt(document.forms["myform"]["Paid"].value);   
 }
 if(isNaN(document.forms["myform"]["Paying"].value) || document.forms["myform"]["Paying"].value=="") {   
 var text4 = 0;   
 } else {   
 var text4 = parseInt(document.forms["myform"]["Paying"].value);   
 }
 document.forms["myform"]["Balance"].value =(text1-text4)-(<?php echo $TotalPaid?>);
 document.forms["myform"]["Paid"].value =((<?php echo $TotalPaid?>)+text4);
 document.forms["myform"]["TotalDue"].value =(text1-text4)-(<?php echo $TotalPaid?>);
 }
 </script>
   <?php } ?>
    </main>
