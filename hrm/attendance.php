...
<nav class="header-nav ms-auto">
  ...
  <main id="main" class="main">
    <span>HRM </span>Payroll And Attendance.
    <div class="card">
      <div class="card-body">
        <div class="row">
          <div class="col">
            ...
            if (isset($_POST['submit'])) {
              ...
              if ($numR2 > 0) {
                while ($row = mysqli_fetch_array($query2)) {
                  $TotalPay = $row['DailyWage'];
                }
                $WagePerPackager = $TotalPay / $Available;
                $sql007 = "INSERT INTO attendance (AttendanceName, Available, WagePerPackager, TotalPay) VALUES ('$AttendanceName','$Available','$WagePerPackager','$TotalPay')";
                $queryy = mysqli_query($conn, $sql007);

                if ($queryy) {
                  echo ("<script LANGUAGE='JavaScript'>
    window.alert('Registered Attendance Successfully');
                ...
                <div class=row>
                  <div class="col-lg-4">
                    ...
                    <select id="dropdown" class="form-control">
                      <option selected="selected" disabled="true">Please Select Casuals</option>
                      <?php
                      include "../connect.php";
                      $sql = mysqli_query($conn, "SELECT * FROM casuals  ORDER BY AddedOn DESC");
                      while ($row = mysqli_fetch_array($sql)) {
                      ?>
                        <option value="<?php echo $row['FirstName'] . " " . $row['LastName']; ?>"><?php echo $row['FirstName'] ?> <?php echo $row['LastName']  ?></option>
                      <?php
                      }
                      ?>
                    </select>
                  ...
                  <div class="row">
                    <div class="card-body ">
                      <textarea type="text" id="ta" ng-model="htmlcontent" style="display: block; height:250px; width: 180px;" name="AttendanceName" required></textarea>
                    </div>

                  </div>
                </div>
                <div class="col-md-14"><input class="btn btn-primary" type="submit" value="Confirm Todays Attendance" name="submit" ONCLICK="return count_words(document.myform.AttendanceName)"></div>
              </div>
          </div>
        </div>
      </div>
    ...
    <script>
      ...
      mydropdown.onchange = function() {
        ...
        mytextbox.value = mytextbox.value + '  ' + mydropdownValue + '\r';
      }
    </script>
    <script>
    ...
    if ($num_rows21 > 0) {
      ...
      echo "<th>Daily Per Packager</th>";
      echo "<th>Total Wage</th>";
