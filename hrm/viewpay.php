...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <script src="assets/js/main.js"></script>
    ...
    include "../connect.php";
                $idc=$_GET['id'];
                $sqls="select * from casuals WHERE Casualid='$idc'";
                ...
                if($num>0){
                    while($rows=mysqli_fetch_array($resul)){
                        ...
                        $CasualName=$rows['FirstName']." ".$rows['LastName'];
...
<main id="main" class="main">
    <span>View Payroll:(Reference No:<?php echo $_GET['id']  ?>)</span>
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col">
                    <div>
                    <label>Customer Name:</label> <?php echo  $CasualName?>
                    </div>
                    <div class="col">
                    <div>
                ...
                <div class="col">
                    <div>
                    <label>Casual Id:</label> <?php echo $idc ?>
                    </div>
                    <div class="col">
                    <div>
...
if($numrow){
    while($row=mysqli_fetch_array($query)){
       ...
       $sql232="select SUM(WagePerPackager) from attendance WHERE AttendanceName LIKE '%$CasualName%'";
       $results232=mysqli_query($conn,$sql232);
       $num232=mysqli_num_rows($results232);
       if($num232){
           while($row=mysqli_fetch_array($results232)){
               $TotalAmount=$row['SUM(WagePerPackager)'];
               $NTotalAmount=$TotalAmount-$TotalPaid;
           }
       $sql4="select * from casuals where Casualid='$id'";
           ...
           if($num>0){ 
                       ...
                       echo "<th style='background-image: linear-gradient(to right, gray , azure);color:white;margin-right:3%; border-radius:12px;float:left; width:40%'>Total Paid:</th>";
                       echo "<th style='background-image: linear-gradient(to right, gray , brown);:gray;color:white;margin-right:3%; border-radius:12px; text-align:center; float:left; width:40%'>Due:</th>";
                       echo "</tr>";
               while($row=mysqli_fetch_array($resul)){
                       echo "<tr style='width:500px'>";
                       echo "<td style='background-color:gray;color:white;margin-right:3%; border-radius:12px; text-align:center; float:left; width:40%'>"." KES ".number_format($row['TotalPaid'])."</td>";
                       echo "<td style='background-color:gray;color:white; border-radius:12px; text-align:center; float:left; width:40%'>"." KES ".number_format($NTotalAmount)."</td>";
                       echo "</tr>";
            }
            echo "</table>";
            echo "</div>";
    ...
    include '../connect.php';
    $id=$_GET['id'];
                $sql="select * from payroll WHERE Casualid='$id'";
                $results=mysqli_query($conn,$sql);
                $num=mysqli_num_rows($results);
                if($num>0){
                  echo "<div class='card bg-light'>";
                  echo "<div class='table-responsive' style='height: 200px;'>";
                    echo "<table class='table table-hover'>";
                    echo "<tr style='background-color:gray;color:white;border-radius:6px;top:0;position:sticky;'>";
                    echo "<th>Reference No:(Payroll)</th>";
                    echo "<th>Invoice No:(payment)</th>";
                    echo "<th>Payments:</th>";
                    ...
                    while($row=mysqli_fetch_array($results)){
                                            ...
                                            echo "<tr>";
                    echo "<td>".$row['Casualid']."</td>";
                    echo "<td>".$row['Payrollid']."</td>";
                    echo "<td>"." KES ".$row['Paying'].' Ksh'."</td>";
                    echo "<td>".$row['Date']."</td>";
                    echo "</tr>";
                    }
                    echo "</table>";
