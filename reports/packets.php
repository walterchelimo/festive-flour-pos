...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <span>Logout</span>
    ...
    <main id="main" class="main">
        <span>Reports </span> Packet Purchases
        <div>
            <div class="card">
                <div class="card-body">
                    <div class="row">
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        $sql = "SELECT * FROM packetpurchase WHERE Date BETWEEN '$range1' AND '$range2' ORDER BY PPurchaseId DESC";
                    ...
                    ?>
                        ...
                        $sql11 = "SELECT * FROM packetpurchase ORDER BY PPurchaseId DESC";
                        $results211 = mysqli_query($conn, $sql11);
                        $num_rows211 = mysqli_num_rows($results211);
                        if ($num_rows211 > 0) {
                            ?>
                                <div class="col-lg-2 col-sm-8">
                                    <input class="form-control" id="myInput" type="text" placeholder="Search...">
                                ...
                                <div id="Print">
                                    ...
                                    while ($row = mysqli_fetch_array($results211)) {
                                        echo "<tbody id='myTable'>";
                                        echo "<tr>";
                                        echo "<td>" . $row['PPurchaseId'] . "</td>";
                                        echo "<td>" . $row['SupplierName'] . "</td>";
                            ...
                            <script>
                                ...
                                function updateSubTotal1() {
                                    ...
                                    let subTotal = Array.from(table.rows).slice(1).reduce((total, row) => {
                                        return total + parseFloat(row.cells[6].innerHTML) * parseFloat(row.cells[5].innerHTML);
                                    }, 0);
                                    document.getElementById("val1").innerHTML = "       Bale Purchases= KES " + subTotal.toFixed(2);
                                }
                                ...
                                function updateSubTotal2() {
                                    var table = document.getElementById("dtVerticalScrollExample");
                                    let subTotal = Array.from(table.rows).slice(1).reduce((total, row) => {
                                        return total + parseFloat(row.cells[8].innerHTML) * parseFloat(row.cells[7].innerHTML);
                                    }, 0);
                                    document.getElementById("val2").innerHTML = "      1KG Purchases= KES " + subTotal.toFixed(2);
                                }
                                ...
                                function updateSubTotal3() {
                                    var table = document.getElementById("dtVerticalScrollExample");
                                    let subTotal = Array.from(table.rows).slice(1).reduce((total, row) => {
                                        return total + parseFloat(row.cells[10].innerHTML) * parseFloat(row.cells[9].innerHTML);
                                    }, 0);
                                    document.getElementById("val3").innerHTML = "       2KG Purchases= KES " + subTotal.toFixed(2);
                                }

                                updateSubTotal4(); // Initial call
                                function updateSubTotal4() {
                                    var table = document.getElementById("dtVerticalScrollExample");
                                    let subTotal = Array.from(table.rows).slice(1).reduce((total, row) => {
                                        return total + (parseFloat(row.cells[10].innerHTML) * parseFloat(row.cells[9].innerHTML)) + (parseFloat(row.cells[8].innerHTML) * parseFloat(row.cells[7].innerHTML)) + (parseFloat(row.cells[6].innerHTML) * parseFloat(row.cells[5].innerHTML));
                                    }, 0);
                                    document.getElementById("val4").innerHTML = "       Total Amount of Purchases = KES " + subTotal.toFixed(2);
                                }
                            </script>
    </main>
</body>
