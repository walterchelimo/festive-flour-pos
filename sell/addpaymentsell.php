...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <script src="assets/js/main.js"></script>
    ...
    include "../connect.php";
    $id=$_GET['id'];
                $sql="select * from sales WHERE Sellid='$id'";
                $results=mysqli_query($conn,$sql);
                $num=mysqli_num_rows($results);
                if($num){
                    while($row=mysqli_fetch_array($results)){
                        $TotalAmount=$row['TotalSell'];
                        $Due=$row['Due'];
                ...
                $sql2="UPDATE sales SET Due='$TotalDue',PaymentAmount='$Paid' WHERE Sellid='$id'";
                $res=mysqli_query($conn,$sql2);
                if($res){
                    $sql3="INSERT INTO addpayment (Paying,Balance,Sellid) VALUES('$Paying','$TotalDue','$id')";
                    $result3=mysqli_query($conn,$sql3);
                     echo "<script>window.alert('Payment Added successfully');
    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="row">
                ...
                <input type="number" step="0.01" class="form-control" name="TotalAmount" value="<?php echo $TotalAmount  ?>" readonly onkeyup="calculate(this.value)">
                </div>
                <div class="col-lg-4 col-sm-12">
                <label>Total Due(Ksh)<caps>*</caps></label>
                <input type="number" step="0.01" class="form-control" name="TotalDue" value="<?php echo $Due  ?>" readonly onkeyup="calculate(this.value)">
                </div>
                 <div class="col-lg-4 col-sm-12">
                <label>Paid(Ksh)<caps>*</caps></label>
                <input type="number" step="0.01" class="form-control" name="Paid" value="<?php echo $Paid  ?>" readonly onkeyup="calculate(this.value)">
                </div>
                <div class="col-lg-4 col-sm-12">
                <label>Paying Amount(Ksh)<caps>*</caps></label>
                <input type="number" step="0.01" class="form-control" name="Paying" onkeyup="calculate(this.value)">
                </div>
                <div class="col-lg-4 col-sm-12">
                <label>Balance Due:</label>
                <input type="number" step="0.01" class="form-control" name="Balance" onkeyup="calculate(this.value)">
                    </div>
            </div>
                            <div>
...
if(isNaN(document.forms["myform"]["TotalAmount"].value) || document.forms["myform"]["TotalAmount"].value=="") {   
 ...
 document.forms["myform"]["Balance"].value =(text2-text4);
 document.forms["myform"]["Paid"].value =((<?php echo $Paid  ?>)+text4);
 document.forms["myform"]["Due"].value =(text2-text4);
 }
