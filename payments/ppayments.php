...
<a class="nav-link collapsed" href="../logout.php">
  ...
  <main id="main" class="main">
    <span>Payment Account </span>Purchases.
    <div class="card">
        <div class="card-body">
                <div class="row">
            <div class="col">
...
<?php 
    include "../connect.php";
    $log7="Select SUM(AmountDue), SUM(AmountPaid) FROM purchases WHERE PaymentMethod='Cash'";
    $res7=mysqli_query($conn,$log7);
    if($res7){
        while($row=mysqli_fetch_array($res7)){
            $TotalDue=$row['SUM(AmountDue)'];
            $TotalPaid=$row['SUM(AmountPaid)'];
            $Total=$TotalPaid+$TotalDue;
        }
        $log8="UPDATE payments SET TotalPaid='$TotalPaid', TotalDue='$TotalDue', Total='$Total' WHERE PaymentMethod='Cash'";
...
$log9="Select SUM(AmountDue), SUM(AmountPaid) FROM purchases WHERE PaymentMethod='Mpesa'";
...
if($res9){
    ...
    $log10="UPDATE payments SET TotalPaid='$TotalPaid', TotalDue='$TotalDue', Total='$Total' WHERE PaymentMethod='Mpesa';";
...
$log10="Select SUM(AmountDue), SUM(AmountPaid) FROM purchases WHERE PaymentMethod='BankTransfer'";
$res9=mysqli_query($conn,$log10);
if($res9){
    while($row=mysqli_fetch_array($res9)){
        $TotalDue=$row['SUM(AmountDue)'];
        $TotalPaid=$row['SUM(AmountPaid)'];
    ...
    $log10="UPDATE payments SET TotalPaid='$TotalPaid', TotalDue='$TotalDue', Total='$Total' WHERE PaymentMethod='BankTransfer'";
    $res10=mysqli_query($conn,$log10);
    if($res10){
        echo "";
       }
...
$log11="Select SUM(AmountDue), SUM(AmountPaid) FROM purchases WHERE PaymentMethod='Cheque'";
...
if($res11){
    ...
    $log11="UPDATE payments SET TotalPaid='$TotalPaid', TotalDue='$TotalDue', Total= '$Total' WHERE PaymentMethod='Cheque'";
    $res10=mysqli_query($conn,$log11);
    if($res11){
        echo "";
       }
       else{
    ...
    if($num_rows21>0){
      ...
      echo "<tr style='background-color:white;'>";
           ...
           echo "<th>TotalPaid</th>";
           echo "<th>TotalDue</th>";
           echo "<th>Total Expected</th>";
           echo "</tr>";
