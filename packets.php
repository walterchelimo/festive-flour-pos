  ...
  <main id="main" class="main">
                  <div>
                      ...
                      <span>Packets Available</span>
                             ...
                             <button type="button" class="btn btn-lg" id="btn1" data-toggle="modal" data-target="#myModal" style="float:right; margin-left:5px; background-color:#ddd"><i class="bi-exclamation-triangle text-danger"></i>Alert(qty)</button>
  <!-- Modal -->
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content p-3">
        <div class="modal-header">
        <h2 class="modal-title">Set Alert Quantity <i class="bi-exclamation-triangle text-danger"></i></h2>
        ...
        <form action="" method="POST">
                ...
                <div class="col-lg-4 col-sm-12">
                      <label>Balers:<caps>*</caps><i class="bi-exclamation-triangle text-warning"></i></label>
                      <input type="number" class="form-control" name="ba">
                  </div>     
                  <div class="col-lg-4 col-sm-12">
                  <label>1Kg Packets:<caps>*</caps><i class="bi-exclamation-triangle text-warning"></i></label>
                      <input type="number" class="form-control" name="1kgPacketa">
                  </div>
                  <div class="col-lg-4 col-sm-12">
                  <label>2Kg Packets:<caps>*</caps><i class="bi-exclamation-triangle text-warning"></i></label>
                      <input type="number" class="form-control" name="2kgPacketa">
                  </div>  
                </div>
                <div class="d-flex justify-content-center mt-4">
                    <button type="submit" name="submit" class="btn btn-default" style="background-color: #3c8dbc;color:white;font-size:20px">set</button>
                    </div>          
              </form>
            <?php
            include "connect.php";
            if(isset($_POST['submit'])){
              $balera=$_POST['ba'];
              $onekgPacketa=$_POST['1kgPacketa'];
              $twokgPacketa=$_POST['2kgPacketa'];

              $sqll="UPDATE packets SET ba='$balera', 1kgPacketa='$onekgPacketa', 2kgPacketa='$twokgPacketa'";
              $query=mysqli_query($conn,$sqll);
              if($query){
                echo "<script>alert('Alert set')</script>";
              }
            }
            ...
            <a href="addpackets.php"><button type="button" class="btn btn-info btn-lg" style="float:right;"><i class="bi bi-plus-circle"></i>Add New Packet Purchase</button></a>
            </div>
...
function calculate() {   
 ...
 if(isNaN(document.forms["myform"]["UnitPrice"].value) || document.forms["myform"]["UnitPrice"].value=="") {   
 var text2 = 0;   
 } else {   
 var text2 = parseFloat(document.forms["myform"]["UnitPrice"].value);   
 }   
 ...
 Show  <select name="entries" id="entries">
  ...
  <input class="form-control" id="myInput" type="text" placeholder="Search...">
      ...
      <div class="row">
          <?php 
    include "connect.php";
    $sql="SELECT * FROM packets ORDER BY Packetid DESC ";
    ...
    if($num_rows21>0){ 
          echo "<div class='table-responsive'>";
           ...
           echo "<tr>";
           echo "<th>Bale Packets</th>";
           echo "<th> 1KG Packets</th>";
           echo "<th> 2KG Packets</th>";
           echo "<th>Action</th>";
           echo "</tr>";
     ...
     while($row=mysqli_fetch_array($results21)){
               ...
               echo "<tr>";
               echo "<td>".$row['BalePackets']."</td>" ;
         echo "<td>".$row['1kgPackets']."</td>" ;
         echo "<td>".$row['2kgPackets']."</td>" ;
               echo " <td>
                   
                    
                    <a class='m-2' href='editpackets.php?id=".$row['Packetid']."'><span class='bi bi-pencil-square bg-primary p-2'  style='font-size:20px; border-radius:4px;'>Edit</span> </a>
                    </td>";
...
<div class="card">
  ...
  <div class="row">
          <?php 
    include "connect.php";
    $sql="SELECT * FROM packetpurchase ORDER BY PPurchaseId DESC LIMIT 10 ";
    $results21=mysqli_query($conn, $sql);
    $num_rows21=mysqli_num_rows($results21);
    if($num_rows21>0){ 
          echo "<div class='table-responsive'>";
           ...
           echo "<th>Bales Packets</th>";
           echo "<th>Bales Packet Cost</th>";
           echo "<th>1KG Packets</th>";
           echo "<th>1KG Packet Cost</th>";
           echo "<th>2KG Packets</th>";
           echo "<th>2KG Packet Cost</th>";
         ...
         <td>
                   <div class="dropdown">
                     ...
                     <div class="dropdown-menu">
                       <a class="dropdown-item" href="deletepp.php?ID=<?php echo $row['PPurchaseId']?>"><i class="bi bi-trash"></i>Delete</a>
                       <a class="dropdown-item" href="editpp.php?ID=<?php echo $row['PPurchaseId']?>"><i class="bi bi-pencil-square"></i>Edit</a>
                       <?php
                       if($payedf>0){
                        ?>
                      <a class="dropdown-item" href="addpaypp.php?ID=<?php echo $row['PPurchaseId']?>"><i class="bi bi-cash-coin"></i>add payment</a>
                       <?php
                       }
                       ?>
                        <a class="dropdown-item" href="viewpaypp.php?ID=<?php echo $row['PPurchaseId']?>&supplierid=<?php echo $row['SupplierName']?>"><i class="bi bi-cash-coin"></i>view payment</a>
               ...
               <?php
                              ...
                              <td>
                                  <?php 
                                  $sql1="SELECT * FROM suppliers WHERE SupplierId='$cusname'";
               ...
               echo "<td>".$row['Bales']."</td>" ;
               echo "<td>".$row['BaleCost']."</td>" ;
               echo "<td>".$row['1KGPackets']."</td>" ;
               echo "<td>".$row['1KGPacketCost']."</td>" ;
               echo "<td>".$row['2KGPackets']."</td>" ;
               echo "<td>".$row['2KGPacketCost']."</td>" ;
               echo "<td style='color:green'>".$row['Payment']."</td>" ;
               echo "<td style='color:red'>".$row['TotalDue']."</td>" ;
               ?>
               <td>
               <?php
         $Due=$row['TotalDue'];
