...
<a class="nav-link collapsed" href="logout.php">
  ...
  <style>
    ...
    include "connect.php";
                $ID=$_GET['ID'];
                $sqlp="select * from packetpurchase WHERE PPurchaseId='$ID'";
                ...
                if($num>0)
                {
                    while($row=mysqli_fetch_array($results)){
                        ...
                        $TotalPrice=$row['Payment']+$row['TotalDue'];
                        $AmountPaid=$row['Payment'];
                    }
                }
                if(isset($_POST['update'])){
                    
                    $NewAmountPaid=$_POST['NewAmountPaid'];
                    $Paying=$_POST['PaymentAmount'];
                    $md=$_POST['AmountDue'];

                    $NewAmountDue=$TotalPrice-$NewAmountPaid;
                    
                    $sql="INSERT INTO addpacketpurchase (PPurchaseId,Paying) VALUES('$ID','$Paying')";
                    $results=mysqli_query($conn,$sql);
                    if($results){
                      $update="UPDATE packetpurchase SET Payment='$NewAmountPaid', TotalDue='$NewAmountDue' WHERE PPurchaseId='$ID'";
                      $results1=mysqli_query($conn,$update);
                      echo ("<script LANGUAGE='JavaScript'>
                      window.alert('Paid Successfully Successfully');
                      window.location.href='packets.php';
                      </script>
                      ");
...
<div class="row" style="padding-bottom: 10px;">
    ...
    <div class="col-lg-4 col-sm-12">
        ...
        <div>
        <input type="number" min=1 name="TotalPrice" value="<?php  echo $TotalPrice; ?>" class="form-control"  onkeyup="calculate3(this.value)" readonly>
        </div>
    </div>
    <div class="col-lg-4 col-sm-12">
        <label>Amount Due</label>
        <div>
        <input type="number" min=1 name="AmountDue" class="form-control" value="<?php  echo $AmountDue; ?>"  readonly>
        </div>
    </div>
    <div class="col-lg-4 col-sm-12">
        <label>Amount Paid</label>
        <div>
        <input type="number" min=1 name="AmountPaid" class="form-control" value="<?php  echo $AmountPaid; ?>" onkeyup="calculate(this.value)" readonly>
        </div>
    </div>
    <div class="col-lg-4 col-sm-12">
        <label>Paying Amount</label>
        <div>
        <input type="number"  min=1 name="PaymentAmount" class="form-control" onkeyup="calculate(this.value)" required>
        </div>
    </div>
    ...
    <div class="col-lg-4 col-sm-12">
        ...
        <div>
        <input type="number"  min=1 name="NewAmountPaid" class="form-control" readonly >
        </div>
    </div>
    
                      
      </div>
      <input type="submit" class="form-control" name="update" id="save" value="Submit" style="float:right;">
...
function calculate() {   
 ...
 var text1 = parseInt(document.forms["myform"]["AmountPaid"].value);   
 }   
 if(isNaN(document.forms["myform"]["PaymentAmount"].value) || document.forms["myform"]["PaymentAmount"].value=="") {   
 var text2 = 0;   
 } else {   
 var text2 = parseFloat(document.forms["myform"]["PaymentAmount"].value);   
 }   
 document.forms["myform"]["NewAmountPaid"].value = text1+text2;
 }  
 </script>
      
            <script type = "text/javascript">  
function calculate3() {   
 if(isNaN(document.forms["myform"]["TotalPrice"].value) || document.forms["myform"]["TotalPrice"].value=="") {   
 var text8 = 0;   
 } else {   
 var text8 = parseInt(document.forms["myform"]["TotalPrice"].value);   
 }   
 if(isNaN(document.forms["myform"]["NewAmountPaid"].value) || document.forms["myform"]["NewAmountPaid"].value=="") {   
 var text9 = 0;   
 } else {   
 var text9 = parseFloat(document.forms["myform"]["NewAmountPaid"].value);   
 }   
 document.forms["myform"]["NewAmountDue"].value = text8-text9;
 }  
 </script>
          </main>
    </body>
